...
// Initialize polyscope, including windowing system and openGL. Should be called
// exactly once at the beginning of a program. If initialization fails in any
// way, an exception will be thrown. The backend string sets which rendering
// backend to use. If "", a reasonable default backend will be chosen.
...
// Do shutdown work and quit the entire program. Can be called in other
// situations due to errors (etc)
...
extern std::vector<glm::vec4> stroke;
...
// The default version with name="" arbitrarily returns any structure of that
// type. This is useful as a shorthand when only using a single structure.
...
// De-register a structure, of any type. Also removes any quantities associated
// with the structure
void removeStructure(Structure* structure, bool errorIfAbsent = true);
void removeStructure(std::string type, std::string name,
                     bool errorIfAbsent = true);
void removeStructure(std::string name, bool errorIfAbsent = true);

// De-register all structures, of any type. Also removes any quantities
// associated with the structure
void removeAllStructures();

// Recompute the global state::lengthScale, boundingBox, and center by looping
// over registered structures
void updateStructureExtents();

// Essentially regenerates all state and programs within Polyscope, calling
// refresh() recurisvely on all structures and quantities
void refresh();

// === Handle draw flow, interrupts, and popups

// Main draw call . Note that due to cached drawing, this will draw the 3D
// structures
void draw(bool withUI = true);

// Request that the 3D scene be redrawn for the next frame. Should be called
// anytime something changes in the scene.
void requestRedraw();

// Has a redraw been requested for the next frame?
bool redrawRequested();

// Managed a stack of of contexts to draw the UI. Usually contains one entry,
// which causes the main GUI to be drawn, but in general the top callback will
// be called instead. Primarily exists to manage the ImGUI context, so callbacks
// can create other contexts and circumvent the main draw loop. This is used
// internally to implement messages, element selections, etc.
void pushContext(std::function<void()> callbackFunction,
                 bool drawDefaultUI = true);
void popContext();

// === Utility

// Execute one iteration of the main loop
// Exposed so that some weird flow (eg, errors) can re-enter the main loop when
// appropriate. Be careful!
void mainLoopIteration();
void initializeImGUIContext();
void drawStructures();

// Called to check any options that might have been changed and perform
// appropriate updates. Users generally should not need to call this directly.
