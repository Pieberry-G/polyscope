...
ThemeColor stringToThemeColor(const std::string& colorStr) {
  static const std::unordered_map<std::string, ThemeColor> stringMap = {
      {"Green", ThemeColor::Green},   {"Red", ThemeColor::Red},     {"Blue", ThemeColor::Blue},
      {"Purple", ThemeColor::Purple}, {"Brown", ThemeColor::Brown},
  };
  auto it = stringMap.find(colorStr);
  return it != stringMap.end() ? it->second : ThemeColor::Green; // 默认返回Green
}
