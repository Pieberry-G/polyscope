...
// The context stack should _always_ have at least one context in it. The lowest
// context is the one created at initialization.
struct ContextEntry {
...
void readPrefsFile() {
    ...
    // We never really care if something goes wrong while loading preferences,
    // so eat all exceptions
    catch (...) {
...
void init(std::string backend) {
    ...
    // push a fake context which will never be used (but dodges some
    // invalidation issues)
...
void pushContext(std::function<void()> callbackFunction, bool drawDefaultUI) {
    ...
    ImGui::GetIO() = oldIO; // Copy all of the old IO values to new. With ImGUI 1.76 (and
                            // some previous versions), this was necessary to fix a bug where
                            // keys like delete, etc would break in subcontexts. The problem
                            // was that the key mappings (e.g. GLFW_KEY_BACKSPACE -->
                            // ImGuiKey_Backspace) need to be populated in io.KeyMap, and
                            // these entries would get lost on creating a new context.
    ...
    if (contextStack.size() > 50) {
        ...
        throw std::runtime_error("Uh oh, polyscope::show() was recusively MANY "
                                 "times (depth > 50), this "
                                 "is probably a bug. Perhaps "
                                 "you are accidentally calling show every time "
    ...
    oldIO = ImGui::GetIO(); // Copy new IO values to old. I haven't encountered
                            // anything that strictly requires this, but it feels
                            // like we should mirror the behavior from pushing.
...
void processInputEvents() {
    ...
    if (!io.WantCaptureMouse && !widgetCapturedMouse) {
        ...
        if (dragLeft || dragRight) {
            ...
            // Changed by cyh
            if (render::engine->isKeyPressed('q')) {
                ...
                {
                  pick::updatePosFramebuffer();
                  glm::vec4 pickPos = pick::pickPosQuery(io.DisplayFramebufferScale.x * p.x, io.DisplayFramebufferScale.y * p.y);
                  if (!state::stroke.empty())
                  {
                    float distance = glm::length(glm::vec3(pickPos - state::stroke.back()));
                    if (distance > 0.02f && distance < 0.1f) {
                      state::stroke.push_back(pickPos);
                      showStroke();
                    }
                  } else {
                    state::stroke.push_back(pickPos);
...
void renderScene() {
    ...
    if (render::engine->getTransparencyMode() == TransparencyMode::Pretty) {
        ...
        // We will perform several "peeled" rounds of rendering in to the usual
        // scene buffer. After each, we will manually composite in to the final
        // scene buffer.


        // Clear the final buffer explicitly since we will gradually composite
        // in to it rather than just blitting directly as in normal rendering.
        ...
        render::engine->setDepthMode(); // we need depth to be enabled for the
                                        // clear below to do anything
...
void draw(bool withUI) {
    ...
    if (withUI) {
        if (contextStack.back().drawDefaultUI) {
            ...
            // Note: It is important to build the user GUI first, because it is
            // likely that callbacks there will modify polyscope data. If we do
            // these modifications happen later in the render cycle, they might
            // invalidate data which is necessary when ImGui::Render() happens
            // below.
    ...
    // This callback is Polyscope implementation detail, which is distinct from
    // the userCallback (which gets called below)
...
void mainLoopIteration() {
    ...
    // The windowing system will let this busy-loop in some situations,
    // unfortunately. Make sure that doesn't happen.
    if (options::maxFPS != -1) {
...
void show(size_t forFrames) {
    ...
    if (!state::initialized) {
        throw std::logic_error(options::printPrefix + "must initialize Polyscope with polyscope::init() "
                                                      "before calling polyscope::show().");
    }
...
Structure* getStructure(std::string type, std::string name) {
    ...
    if (name == "") {
        if (sMap.size() != 1) {
            error("Cannot use automatic structure get with empty name unless "
                  "there "
                  "is exactly one structure of that type "
                  "registered");
            return nullptr;
        }
        ...
        for (auto entry : typeMap.second) {
            ...
            if (entry.first == name) {
                if (targetStruct == nullptr) {
                    ...
                    error("Cannot use automatic structure remove with empty "
                          "name unless "
                          "there is exactly one structure of that "
                          "type registered. Found two structures of different "
                          "types with "
                          "that name: " +
...
void processLazyProperties() {
    ...
    // Note: This function essentially represents lazy software design, and it's
    // an ugly and error-prone part of the system. The reason for it that some
    // settings require action on a change (e..g re-drawing the scene), but we
    // want to allow variable-set syntax like `polyscope::setting = newVal;`
    // rather than getters and setters like `polyscope::setSetting(newVal);`. It
    // might have been better to simple set options with setter from the start,
    // but that ship has sailed.
    //
    // This function is a workaround which polls for changes to options
    // settings, and performs any necessary additional work.
    ...
    if (lazy::groundPlaneEnabled != options::groundPlaneEnabled || lazy::groundPlaneMode != options::groundPlaneMode) {
        ...
        if (!options::groundPlaneEnabled) {
            // if the (depecated) groundPlaneEnabled = false, set mode to None,
            // so we only have one variable to check
...
void updateStructureExtents() {
    ...
    // If we got a bounding box but not a length scale we can use the size of
    // the box as a scale. If we got neither, we'll end up with a constant near
    // 1 due to the above correction
