...
void Engine::allocateGlobalBuffersAndPrograms() {
  ...
  { // Pick buffer
   ...
   { // Pos buffer
    posColorBuffer = generateRenderBuffer(RenderBufferType::Float4, view::bufferWidth, view::bufferHeight);
    posDepthBuffer = generateRenderBuffer(RenderBufferType::Depth, view::bufferWidth, view::bufferHeight);

    posFramebuffer = generateFrameBuffer(view::bufferWidth, view::bufferHeight);
    posFramebuffer->addColorBuffer(posColorBuffer);
    posFramebuffer->addDepthBuffer(posDepthBuffer);
    posFramebuffer->setDrawBuffers();
  }
